...
impl Contract {
    ...
    pub fn nft_mint(&mut self, element: u8, receiver_id: AccountId, perpetual_royalties: Option<HashMap<AccountId, u32>>) {
        ...
        assert!(deposit >= 1 , "Attached deposit must = 1 to buy NFT Ticket" );
         // transfer deposit to zoo wallet
        Promise::new("zoo-wallet-fund-auction.louiskate.testnet".to_owned()).transfer(deposit);
        ...
        let media: Option<String> = match element {
            0 => Some("https://bafkreicgmhs26n7x4oqpyscs7vretndlm4jwdxusyw5ftgg55mb3vl3zde.ipfs.nftstorage.link/".to_owned()),
            1 => Some("https://bafybeihse7c3gxt273e2paeefpv4w2swn2xiuyp2wbu7uuforxfrgn6ugq.ipfs.nftstorage.link/".to_owned()),
            2 => Some("https://bafybeibmcg7mdznzm5edf5jtqkcbzvekh7k2mhdk3ohnxbcr7pkphibf6q.ipfs.nftstorage.link/".to_owned()),
            _ => Some("https://bafkreifje63g66tttsomrgopbhb2bzvqczby7f5ycrxy5usk2pcwyj7b74.ipfs.nftstorage.link/".to_owned())
        };

        let metadata = TokenMetadata {
            title: Some("Ticket ID: ".to_string() + &token_id.to_string() + &" | Vip: ".to_string() + &quality.unwrap().to_string()), 
            description: Some("Vip: ".to_string() + &quality.unwrap().to_string()), 
            media: media, 
            media_hash: None, 
            copies: None, 
            issued_at: None, 
